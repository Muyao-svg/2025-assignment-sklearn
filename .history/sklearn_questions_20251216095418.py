...
class MonthlySplit(BaseCrossValidator):
    ...
    def get_n_splits(self, X, y=None, groups=None):
        ...
        if isinstance(X, pd.DataFrame):
            ...
            months = time_index.to_period('M').unique()  # 按月分段
            return len(months) - 1  # 返回月份数量减去1，因为我们需要前一个月做训练，当前月做预测
        else:
    ...
    def split(self, X, y, groups=None):
        ...
        if isinstance(X, pd.DataFrame):
            ...
            for i in range(len(months) - 1):
                # 当前月份的训练集是之前一个月
                idx_train = time_index[time_index.to_period('M') == months[i]].index
                # 下一个月的测试集
                idx_test = time_index[time_index.to_period('M') == months[i + 1]].index
